<template>
  ...
  :class="
    isGroupActive(group) && !group.items
      ? 'bg-gray-100 dark:bg-gray-875 border-s-4 border-gray-800 dark:border-gray-100 text-black'
      : ''
  "
...
>
  ...
  />
  <div
    class="ms-2 text-lg group-hover:text-black"
    :class="{
      'text-black': isGroupActive(group) && !group.items,
      'text-white': !(isGroupActive(group) && !group.items),
    }"
  >
    {{ group.label }}
  </div>
        ...
        <div v-if="group.items && isGroupActive(group)">
          <div
            ...
            "
            :class="{
              'bg-gray-100 dark:bg-gray-875 border-s-4 border-gray-800 dark:border-gray-100 text-black': isItemActive(item),
              'text-white hover:text-black': !isItemActive(item)
            }"
    ...
    <div class="window-no-drag flex flex-col gap-2 py-2 px-4">
      ...
      <p
        v-if="showDevMode"
        class="text-xs text-white select-none cursor-pointer"
...
export default defineComponent({
  ...
  methods: {
    ...
    toggleSidebar,
    openDocumentation() {
      ipc.openLink('https://cnit-solutions.com/' + docsPathRef.value);
    },
    setActiveGroup() {
