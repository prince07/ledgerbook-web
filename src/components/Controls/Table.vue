<template>
  <div v-if="tableFields?.length">
    <div v-if="showLabel" class="text-black dark:text-black text-sm mb-1">
      {{ df.label }}
    </div>
    ...
    <div :class="border ? 'border dark:border-gray-800 rounded-md' : ''">
      ...
      <Row
        ...
        "
      >
        <div class="flex items-center ps-2 text-black">#</div>
        <div
          v-for="df in tableFields"
          :key="df.fieldname"
          class="items-center flex px-2 h-row-mid text-black"
          :class="{
      ...
      >
        <div class="flex items-center ps-1">
          <feather-icon name="plus" class="w-4 h-4 text-black" />
        </div>
        <div
